    ...
    class CSF_Field_upload extends CSF_Fields
    {
        ...
        public function render()
        {
            ...
            if ( ! empty( $args['preview'] ) ) {
                $imageUrl = is_numeric($this->value) ? wp_get_attachment_image_url($this->value) : $this->value;
                $preview_type   = ( ! empty( $imageUrl ) ) ? strtolower( substr( strrchr( $imageUrl, '.' ), 1 ) ) : '';
                $preview_src    = ( ! empty( $preview_type ) && in_array( $preview_type, array( 'jpg', 'jpeg', 'gif', 'png', 'svg', 'webp' ) ) ) ? $imageUrl : '';
                ...
                echo '<div class="csf--preview'. esc_attr( $preview_hidden ) .'" style="width: 100%;">';
                echo '<div class="csf-image-preview"'. $preview_style .'>';
                echo '<span><img src="'. esc_url( $preview_src ) .'" class="csf--src" /></span>';
                echo '</div>';
                echo '</div>';

            }
            $customClasses = [
                'class' =>  $args['save_id'] === true ? 'media-id' : ''
            ];

            $fieldType = isset($args['hide_input']) && $args['hide_input'] ? 'hidden' : 'text';
            echo '<input type="' . $fieldType . '" name="' . esc_attr($this->field_name()) . '" value="' . esc_attr($this->value) . '"' . $this->field_attributes($customClasses) . '/>';
